...
# [ch]      2. nb_bins, batch_size, nb_training, nb_eval, std_threshold, scale, verbose, don't care, leave as default.
# [ch]      3. nb_variance_estimate, don't care, might delete later.
def z_min_var(factors, codes, continuous_factors=True, nb_bins=10, batch_size=200, nb_training=800,
    ...
    :param factors:                         dataset of factors [ch] expert features
    ...
    [CH] assume both factors and codes are minmax normalized to (0,1)
    ...
    # [ch] dimension for expert and learned feature
    nb_factors = factors.shape[1]   
    nb_codes = codes.shape[1]
    
    # quantize factors if they are continuous
    if continuous_factors:
        factors = minmax_scale(factors)             # [ch] normalize in [0, 1] all columns
        factors = get_bin_index(factors, nb_bins)   # [ch] quantize values and get indexes
    
    # compute an estimation of empirical variance for each latent dimension
    lines_idx = np.arange(codes.shape[0]) 
    np.random.shuffle(lines_idx)                            # [ch] shuffle the batch learned feature data
    var_codes = codes[lines_idx][:nb_variance_estimate]     # [ch] take the first nb_variance_estimate dimensions
    emp_variances = _compute_variances(var_codes, axis=0)   # [ch] returns "nb_variance_estimate"-dim vector
    
    # identify active latent dimensions, active_dims is a mask
    ...
    classifier = np.argmax(training_votes, axis=1)  # (nb_codes, ) in range of factor's dim
...
def _compute_variances(representation, axis):
    ...
    [ch] Compute the variance for each dimension
...
def _prepare_datasets(factors, codes, batch_size, nb_training, nb_eval, variances, active_dims, verbose):
    ...
    # [ch] {dim_1: #(data that chose it as fixed), dim_2: #(data that chose it as fixed) ... }
    ...
    for factor_id in range(nb_factors):
        ...
        # [ch] gather all possible (discrete) values for the "factor_id"-th dimension, make sure at least 2 data points has that value
        ...
        # [ch] create a list of fixed factor values of length factor_id_count drawn from the set available_factor_values
        # [ch] then split to train, eval
        ...
        # [ch] dict of size: available_factor_values, and assign the count for each possible factor values
        ...
        for factor_value, count in fixed_factor_values.items(): 
            ...
            # [ch] why do we want batch_size * count? 
            # [ch] sample a batch_size of data where the "factor_id"-dim is equal to factor_value 
            nb_examples_needed = batch_size * count
            batch = np.zeros(nb_examples_needed, dtype='int64') # [ch] stores index later
            ...
            for idx in range(0, codes_train.shape[0], batch_size):
                ...
                # [ch] smallest variance within a batch is at argmin dim along the learned feature dimensions
                train_set[line_idx_train, 0] = argmin

                # [ch] the actual factor_id being fixed along the expert feature dimensions
